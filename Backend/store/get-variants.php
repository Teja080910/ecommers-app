...
/* FETCH VARIANTS (scoped to this seller's own product) */

$stmt = $pdo->prepare(

    "SELECT product_varients.*
     FROM product_varients
     INNER JOIN products ON products.id = product_varients.product_id
     WHERE product_varients.product_id=?
     AND products.seller_id=?
     ORDER BY product_varients.id DESC"
);
